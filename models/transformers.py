...
class Transformer(nn.Module):
    def __init__(
        ...
        #设置文本编码器，默认为roberta
        self.text_encoder = RobertaModel.from_pretrained('./models/cache_dir/ssss')
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            # class_mask = class_data.mask.ne(1).bool()

            r_masks = torch.cat([refer_data.mask, knowl_data.mask], dim=1
                                ).unsqueeze(1).repeat(1, len_refer, 1)

            k_masks = torch.cat([ refer_data.mask, knowl_data.mask], dim=1
                                ).unsqueeze(1).repeat(1, len_know, 1)

            text_tensors = torch.cat([refer_data.tensors, knowl_data.tensors], dim=1)
            text_masks = torch.cat([r_masks,k_masks], dim=1)
            text_output = self.text_encoder(input_ids=text_tensors,
                                            attention_mask=text_masks,
                                            output_attentions = True,
                                            )
            encoded_text = text_output.last_hidden_state
            ...
            refer_output = self.text_encoder(input_ids=refer_data.tensors,
                                            attention_mask=refer_data.mask,
                                            output_attentions = True,
                                            )
            encoded_refer = refer_output.last_hidden_state
            o_refer_memory = self.resizer(encoded_refer).transpose(0, 1)



            refer_memory = self.resloter.layers[0](refer_memory, torch.cat([o_refer_memory,knowl_memory],dim=0),
                                                   torch.cat([o_refer_memory, knowl_memory], dim=0),
                                                   tgt_key_padding_mask=refer_mask,
                                                   memory_key_padding_mask=torch.cat([refer_mask,knowl_mask],dim=1),
                                                   )

            refer_memory = self.resloter.layers[1](refer_memory, torch.cat([o_refer_memory,knowl_memory],dim=0),
                                                   torch.cat([o_refer_memory, knowl_memory], dim=0),
                                                   tgt_key_padding_mask=refer_mask,
                                                   memory_key_padding_mask=torch.cat([refer_mask,knowl_mask],dim=1),
                                                   )

            #refer_memory = self.resloter.layers[2](refer_memory, knowl_memory, knowl_memory,
            #                           tgt_key_padding_mask=refer_mask,
            #                           memory_key_padding_mask=knowl_mask,
             #                          )

            srck = torch.cat([src,o_refer_memory,knowl_memory],
                             dim=0)  # [HW+batch_max_sequence_len,bs,d_model]
            maskk = torch.cat([mask,refer_mask, knowl_mask],
                              dim=1)  # [bs,HW+batch_max_sequence_len]
            pos_embedk = torch.cat([pos_embed,
                                    torch.zeros_like(refer_memory),

                                    torch.zeros_like(knowl_memory),
                                    ], dim=0)
            srck, kv_attk = self.kv_encoder.layers[0](srck, src_key_padding_mask=maskk,
                                              pos=pos_embedk)

            # refer_memory = self.resloter.layers[0](refer_memory, knowl_memory,src[len_src:],
            #
            #                                        tgt_key_padding_mask=refer_mask,
            #                                        memory_key_padding_mask=knowl_mask,
            #                                        )
            # knowl_memory =src[len_src:]

            srck, kv_attk = self.kv_encoder.layers[1](srck, src_key_padding_mask=maskk,
                                                  pos=pos_embedk)


            # refer_memory = self.resloter.layers[1](refer_memory, knowl_memory, src[len_src:],
            #
            #                                        tgt_key_padding_mask=refer_mask,
            #                                        memory_key_padding_mask=knowl_mask,
            #                                        )
            ...
            #
            srck, kv_attk = self.kv_encoder.layers[2](srck, src_key_padding_mask=maskk,
                                                  pos=pos_embedk)
            #


            refer_memory = self.resloter.layers[2](refer_memory, srck[len_src:], srck[len_src:],
            ...
            #
            src = srck[:len_src]
            pos_embed = pos_embedk[:len_src]
            mask = maskk[:, :len_src]


            src = torch.cat([src, refer_memory],
                            dim=0)  # [HW+batch_max_sequence_len,bs,d_model]
            mask = torch.cat([mask, refer_mask],
                             dim=1)  # [bs,HW+batch_max_sequence_len]
            pos_embed = torch.cat([pos_embed, torch.zeros_like(refer_memory),
                                   ], dim=0)

            #
            # src, eatt1 = self.encoder.layers[0](src, src_key_padding_mask=mask,
            #                                    pos=pos_embed)
            # src, eatt = self.encoder.layers[1](src, src_key_padding_mask=mask,
            #                                    pos=pos_embed)
            # src, eatt3 = self.encoder.layers[2](src, src_key_padding_mask=mask,
            #                                    pos=pos_embed)
            src, eatt1 = self.encoder.layers[3](src, src_key_padding_mask=mask,
                                               pos=pos_embed)
            src, eatt = self.encoder.layers[4](src, src_key_padding_mask=mask,
                                               pos=pos_embed)
            src, eatt3 = self.encoder.layers[5](src, src_key_padding_mask=mask,
                                               pos=pos_embed)



            img_memory = src[:len_src]
            pos_embed = pos_embed[:len_src]
            mask = mask[:, :len_src]
            refer_query_embed = refer_memory
            # src[:len_src]

            if self.pass_pos_and_query:
                r_tgt = torch.zeros_like(refer_query_embed)
            else:
                r_tgt, refer_query_embed, = refer_query_embed, None
            ...
            memory_cache = {
                ...
                "pos_embed": pos_embed,
                ##[HW+batch_max_sequence_len,bs,d_model]，主要是对图像中像素点的位置编码，batch_max_sequence_len对应的文本编码部分都是0
                "r_query_embed": refer_query_embed,  # [n_query,bs,d_model],
                "attention_shape": (h, w),
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(
        ...
        memory_key_padding_mask: Optional[Tensor] = None, # #[bs,HW+batch_max_sequence_len] ，表明memory哪些元素是真实的，哪些是pad的
