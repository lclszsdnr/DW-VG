...
def adjust_learning_rate(
    ...
    if args.schedule == "step":
        ...
        milestones = list(range(args.lr_drop, args.epochs, 20))
        gamma = 0.5 ** bisect_right(milestones, epoch)
        text_encoder_gamma = gamma
        retriever_gamma = gamma


        # if curr_step < num_warmup_steps:
        #     text_encoder_gamma = gamma
        #     retriever_gamma = float(curr_step) / float(max(1, num_warmup_steps))
        # else:
    ...
    base_lrs = [args.lr, args.lr_backbone, args.text_encoder_lr,args.lr_retriver]
    gammas = [gamma, gamma, text_encoder_gamma,retriever_gamma]
