...
def crop(image, target, region):
    # crop函数可以在指定位置和输出尺寸裁剪给定图像，region需要包含
    # top: int, left: int, height: int, width: in四个值
    ...
    ## 因为图片被裁剪了，将box的坐标也需要进行变化，变换成相对于新图片的坐标
    if "boxes" in target:
        boxes = target["boxes"]
        max_size = torch.as_tensor([w, h], dtype=torch.float32)
        cropped_boxes = boxes - torch.as_tensor([j, i, j, i])
        # torch.min是取两个矩阵中各位置最小的值组成新的矩阵
...
def hflip(image, target):
    ...
    if "refer" in target:
        refer = target["refer"].replace("left", "[TMP]").replace("right", "left").replace("[TMP]", "right")
        target["refer"] = refer
